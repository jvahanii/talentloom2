...
function Analytics() {
  ...
  const cands = useQuery({
    ...
    enabled: !!orgId,
    queryFn: async () =>
      (await supabase.from("candidates").select("*").eq("org_id", orgId!)).data ?? [],
  });
  const hist = useQuery({
    queryKey: ["all-history", orgId],
    enabled: !!orgId,
    queryFn: async () =>
      (await supabase.from("stage_history").select("*").eq("org_id", orgId!).order("changed_at"))
        .data ?? [],
  });
  const reqs = useQuery({
    queryKey: ["reqs", orgId],
    enabled: !!orgId,
    queryFn: async () =>
      (await supabase.from("requisitions").select("*").eq("org_id", orgId!)).data ?? [],
  });

  const funnel = STAGES.map((s) => ({
    stage: STAGE_LABEL[s],
    count: (cands.data ?? []).filter((c) => c.stage === s).length,
  }));
  ...
  (hist.data ?? []).forEach((h) => {
    (byCand[h.candidate_id] ||= []).push({
      to_stage: h.to_stage as Stage,
  ...
  Object.values(byCand).forEach((events) => {
    for (let i = 0; i < events.length - 1; i++) {
      const days =
        (new Date(events[i + 1].changed_at).getTime() - new Date(events[i].changed_at).getTime()) /
        86400000;
  ...
  return (
    <div>
      ...
      <p className="text-sm text-muted-foreground">
        A quick read on your hiring momentum and candidate experience.
      </p>
